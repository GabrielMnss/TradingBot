...
public class Backtest
{
    ...
    public List<decimal> balanceHistoric = new();
    ...
    private void CheckPositionsStatus()
    {
        for (int i = positions.Count - 1; i >= 0; i--)
        {
            ...
            var brPrice = (position.entryPrice + position.tpPrice) * 0.25m;
            ...
            if (position.type == "b")
            {
                if (quotes.Last().Low <= slPrice)
                    Loose(position);
                else if (quotes.Last().High >= tpPrice) Win(position);
                else if (quotes.Last().High >= brPrice && position.status != 'x')
                {
                    position.status = 'x';
            ...
            else
            {
                if (quotes.Last().High >= slPrice)
                    Loose(position);
                else if (quotes.Last().Low <= tpPrice) Win(position);
                else if (quotes.Last().Low <= brPrice && position.status != 'x')
                {
                    position.status = 'x';
                    position.slPrice = position.entryPrice;
                }
    ...
    private void Loose(Position position)
    {
        ...
        else
        {
            ...
            print($"BreakEven   (-0$)    ", ConsoleColor.DarkMagenta);
            print($"[{quotes.Last().Date}]", ConsoleColor.White);
            print($"    {balance:0.00}$\n", ConsoleColor.Yellow);
        }
